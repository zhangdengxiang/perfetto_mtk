...
function recordMenu(recMgr: RecordingManager, routePage: string) {
  ...
  const sbeProbe = m(
      'a[href="#!/record/sbe"]',
      m(
        `li${routePage === 'sbe' ? '.active' : ''}`,
        m('i.material-icons', 'settings'),
        m('.title', 'SBE Tracing Config'),
        m('.sub', 'SBE status information'),
      ),
   );
  ...
  if (isLinuxTarget(target)) {
    probes.push(cpuProbe, powerProbe, memoryProbe, chromeProbe, advancedProbe, sbeProbe);
